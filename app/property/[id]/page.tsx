...
export default function PropertyDetails() {
  ...
  const handleShare = async () => {
    const url = typeof window !== "undefined" ? `${window.location.origin}/property/${property.id}` : `/property/${property.id}`;
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 mb-16">
           ...
           <div className="flex flex-col justify-center">
              ...
              <div className="grid grid-cols-3 gap-4">
                 ...
                 <button 
                    onClick={handleShare}
                    className="bg-transparent border border-slate-300 text-slate-900 hover:bg-slate-50 py-4 rounded-lg font-bold transition-all flex items-center justify-center gap-2"
                 >
                  ...
                  >
                     <div className="relative h-64 bg-slate-100 rounded-2xl overflow-hidden mb-4">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            const url = typeof window !== "undefined" ? `${window.location.origin}/property/${prop.id}` : `/property/${prop.id}`;
                            // @ts-ignore
                            if (navigator.share) {
                              // @ts-ignore
                              navigator.share({ title: prop.title || "Property", url });
                            } else {
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 z-50 flex gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
         ...
         <button 
           onClick={handleShare}
           className="flex-1 border border-slate-300 text-slate-900 bg-white py-3 rounded-xl font-bold text-sm"
         >
           Share
         </button>
      </div>
