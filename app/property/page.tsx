...
function PropertySearchContent() {
  ...
  const shareProperty = async (e: React.MouseEvent, propertyId: string, title: string) => {
    e.stopPropagation();
    const url = typeof window !== "undefined" ? `${window.location.origin}/property/${propertyId}` : `/property/${propertyId}`;
    try {
      // @ts-ignore
      if (navigator.share) {
        // @ts-ignore
                ...
                >
                  <div className="relative h-48 w-full">
                    ...
                    <div className="absolute top-3 left-3 bg-blue-600 text-white px-2 py-1 rounded text-xs font-medium z-10">
                      ...
                      <button
                        onClick={(e) => shareProperty(e, property.id, property.title || "Property")}
                        className="absolute top-3 right-14 p-2 rounded-full bg-white/80 hover:bg-white transition-colors shadow-sm z-10"
                      >
