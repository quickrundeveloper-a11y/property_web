...
function HomeContentInner({ initialTab, showHero = true }: { initialTab?: string, showHero?: boolean }) {
  ...
  const scrollToFilter = (tab?: string) => {
    if (tab) {
      ...
      const newLookingTo = (tab === 'Rent') ? 'rent' : (tab === 'Buy' ? 'sell' : (tab === 'All' ? 'all' : 'sell'));
  ...
  const isPlotType = (item: Property) => {
    ...
    const u = String(item?.units || item?.areaUnit || item?.area_unit || '').toLowerCase();
    
    const landUnits = ['acre', 'bigha', 'hectare', 'kanal', 'biswa', 'guntha', 'kottah', 'ground', 'are', 'rood', 'chatak', 'perch'];
    const hasLandUnit = landUnits.some(unit => u.includes(unit));

    return cat.includes('land') || cat.includes('plot') || t.includes('plot') || hasLandUnit;
  };
  ...
  const matchesActiveType = (item: Property) => {
    ...
    if (activeTab === 'Buy') return isSale;
                  ...
                  >
                    ...
                    <div className="p-5 pt-6 sm:pt-8">
                      ...
                      <div className="flex flex-wrap gap-y-2 items-center justify-between text-gray-500">
                        {!isPlotType(property) && (
                          <>
                            {(Number(property.bedrooms || property.beds || 0) > 0) && (
                            <div className="flex items-center gap-1.5">
                              <Bed className="w-4 h-4 text-[#0066FF]" />
                              <span className="text-xs"><span className="font-bold text-gray-700">{property.bedrooms || property.beds}</span> Beds</span>
                            </div>
                            )}
                            {(Number(property.bathrooms || property.baths || 0) > 0) && (
                            <div className="flex items-center gap-1.5">
                              <Bath className="w-4 h-4 text-[#0066FF]" />
                              <span className="text-xs"><span className="font-bold text-gray-700">{property.bathrooms || property.baths}</span> Bathrooms</span>
                            </div>
                            )}
                          </>
                        )}
